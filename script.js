...
function pintarBarra(id_barra, cantidad, indice, interval){
    ...
    if(x < cantidad){
        ...
        elementos[x].style.backgroundColor = "#2b3457";
    }else{
...
// Agrego el evento al enviar el formulario
form.addEventListener('submit', function(e) {
    e.preventDefault(); // Evita el envío por defecto del formulario

    // Verifico si los campos están llenos
    let nombre = document.getElementById('user_name').value;
    let email = document.getElementById('user_email').value;
    let mensaje = document.getElementById('message').value;

    if (nombre && email && mensaje) {
        mostrarConfirmacion(); // Muestra la ventana modal si todos los campos están llenos
    } else {
...
function mostrarConfirmacion() {
    ...
    mensaje.innerText = "¡Tus datos han sido enviados con éxito!";
